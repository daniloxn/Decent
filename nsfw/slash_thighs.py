...
def Image():
    url = "https://hmtai.hatsunia.cfd/v2/thighs"
...
class SlashThigs(commands.Cog):
    def __init__(self, client):
        self.client = client


    @app_commands.command(name="thighs")
    async def thighs(self, interaction: discord.Interaction):
        ...
        print(f"{interaction.user} things")
    @commands.Cog.listener()
    async def on_ready(self):
        self.client.tree.add_command(self.thighs)
        self.client.tree.sync()

async def setup(client):
    await client.add_cog(SlashThigs(client))