...
def Image():
    url = "https://hmtai.hatsunia.cfd/v2/yuri"
...
class SlashYuri(commands.Cog):
    def __init__(self, client):
        self.client = client


    @app_commands.command(name="yuri")
    async def yuri(self, interaction: discord.Interaction):
        ...
        print(f"{interaction.user} yuri")
    @commands.Cog.listener()
    async def on_ready(self):
        self.client.tree.add_command(self.yuri)
        self.client.tree.sync()

async def setup(client):
    await client.add_cog(SlashYuri(client))