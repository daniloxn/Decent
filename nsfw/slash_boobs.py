...
def Image():
    url = "https://hmtai.hatsunia.cfd/v2/boobs"
...
class SlashBoobs(commands.Cog):
    def __init__(self, client):
        self.client = client


    @app_commands.command(name="boobs", description="Envia uma imagem de Boobs")
    async def boobs(self, interaction: discord.Interaction):
        ...
        if discord.utils.get(interaction.user.roles, name=cargo):
            ...
            print(f"{interaction.user} boobs")
        else:
            embed = discord.Embed(
    ...
    async def on_ready(self):
        self.client.tree.add_command(self.boobs)
        self.client.tree.sync()

async def setup(client):
    await client.add_cog(SlashBoobs(client))