...
def Image():
    url = "https://hmtai.hatsunia.cfd/v2/femdom"
...
class SlashFemdom(commands.Cog):
    def __init__(self, client):
        self.client = client


    @app_commands.command(name="femdom")
    async def femdom(self, interaction: discord.Interaction):
        cargo = "Nsfw"
    ...
    async def on_ready(self):
        self.client.tree.add_command(self.femdom)
        self.client.tree.sync()

async def setup(client):
    await client.add_cog(SlashFemdom(client))