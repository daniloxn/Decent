...
def Image():
    url = "https://hmtai.hatsunia.cfd/v2/ero"
...
class SlashEro(commands.Cog):
    def __init__(self, client):
        self.client = client


    @app_commands.command(name="ero")
    async def ero(self, interaction: discord.Interaction):
        ...
        print(f"{interaction.user} ero")
    @commands.Cog.listener()
    async def on_ready(self):
        self.client.tree.add_command(self.ero)
        self.client.tree.sync()

async def setup(client):
    await client.add_cog(SlashEro(client))