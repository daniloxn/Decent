...
def Image():
    url = "https://hmtai.hatsunia.cfd/v2/manga"
...
class SlashManga(commands.Cog):
    ...
    async def manga(self, interaction: discord.Interaction):
        ...
        print(f"{interaction.user} manga")
    @commands.Cog.listener()
    async def on_ready(self):
        self.client.tree.add_command(self.manga)
        self.client.tree.sync()

async def setup(client):
    await client.add_cog(SlashManga(client))